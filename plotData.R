...
plotData <- function(ymax = NULL, add = F, addDataFileName = "plotData"){
  ...
  data <- data.frame(g = g, popID = rep(0:(length(popIDselU) - 1), nSim + 1), size = size, group = group, scheme = rep(1, length(g)))
  if(add){
  ...
  p <- p + geom_line(aes(alpha = factor(size), size = factor(size), linetype = factor(scheme), group = factor(group), colour = factor(scheme)))
  if(is.null(ymax)){
    p <- p + ylim(min(data$g), max(data$g))
  }else{
    p <- p + ylim(min(data$g), ymax)
  }
  p <- p + scale_alpha_discrete(range = c(0.5, 1), labels = c("Each", "Mean"))
  p <- p + scale_size_manual(name = "", values = c(1, 3), labels = c("Each", "Mean"))
  p <- p + labs(title = "", x = "Generation", y = "Genetic improvement")
  p <- p + guides(alpha = guide_legend("Lines"), size = guide_legend("Lines"))
  p <- p + guides(linetype = guide_legend("Scheme"), colour = guide_legend("Scheme"))
  print(p)
}
